...
@section('li_cotizacion_cielo','active')
@section('title','COTIZACIÓN DE CIELO ACUSTICO')
@section('content')


<form method="POST" action="{{route('cotizacion_cielo.guarda')}}" id="form_cotizacion" parsley-validate novalidate>
	{{csrf_field()}}	
	<div class="col-md-12">
		<!-- Custom Tabs -->
		<div class="nav-tabs-custom">
			...
			<div class="tab-content">
				<div class="tab-pane active" id="tab_1">
					<h3 class="text-center text-info">COTIZACIÓN CIELO ACUSTICO</h3>
					<div class="row">
						<div class="col-md-4">
							<div class="form-group">
								<label for="exampleInputEmail1">INGRESE BASE:</label>
...
<script type="text/javascript">
	function calcular(){
		...
		var route= "{{route('cotizacion_cielo.calcula')}}";
		$.ajax({
			url: route,           
...
<script type="text/javascript">
	function guardar(){
		var base=$('#base').val();
		var altura=$('#altura').val();
		var route= "{{route('cotizacion_cielo.guarda')}}";
		$.ajax({
			url: route,           
			headers: {'X-CSRF-TOKEN': '{{ csrf_token() }}'},
			type: 'POST',     
				...
				$.ajax({
					...
					success: function(data){
						...
						if (data!='FALSE') {
							...
							$('#div_botonera').html('<a href="cotizacion_cielo-pdf/'+data.ID_COT+'" target="_blank" class="btn btn-danger"><i class="fa fa-print"></i> IMPRIMIR PROFORMA GUARDADA</a>');
						}else{
							$.notify({icon:'<i class="fa fa-ban"></i> ',title:' <strong>ERROR!</strong></br>',message:"{{'  Ocurrio un problema al guardar, recargue la Pagina'}}"},{z_index: 2000, type:'danger',animate:{enter:'animated fadeInDown',exit:'animated fadeOutUp'}});
						}
					},
